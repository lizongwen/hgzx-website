...
export default {
  ...
  methods: {
    showDetail: function(current) {
      this.$router.push("/news/index/" + "detail_" + current);
    },
    backList() {
  ...
  beforeRouteEnter(to, from, next) {
    ...
    next(vm => {
      if (to.params.id) {
        vm.showList = false;
        vm.current =
          "newsDetail" + (Number(to.params.id.replace("detail_", "")) + 1);
      } else {
        vm.showList = true;
      }
    });
			...
			return {
				...
				news: [{
					...
					},
					{
						...
						id: "1015",
						date: "2016-11-17",
						title: "国家社会信用体系建设中期评估组一行莅临惠国征信视察指导",
					},
					{
						typeid: 1,
						id: "1016",
...
>>>>>>> 4e70c76ca7f21811a7ad6dda801f835861c82a3f
</script>
